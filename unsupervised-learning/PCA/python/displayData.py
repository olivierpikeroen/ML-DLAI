    ...
    for i in range(n_row):
        ...
        for j in range(n_col):
            ...
            # Empty example to complete the last row if not full
                ...
                img_row = np.concatenate((img_row, np.zeros((example_height, example_width))), axis=1)
            # Padding
            if j < n_col - 1:
                img_row = np.concatenate((img_row, np.zeros((example_height, pad))), axis=1)
        # Concatenate rows
        # First row
        if i == 0:
            img = img_row
        ...
        # Padding
        if i < n_row - 1: 
            img = np.concatenate((img, np.zeros((pad, img_row.shape[1]))), axis=0)
    # Plot
